...
trait InteractsWithCustomMedia
{
    ...
    public function syncMedia(
        ...
        $collect = $collect->map(function ($item, $order) use (
            $collectionName,
            $diskName,
        ) {
    ...
    public function addSingleMedia(
        $singleMedia,
        $collectionName = 'default',
        $diskName = '',
    ): Media {
        if ($singleMedia instanceof UploadedFile) {
            ...
            try {
                $mediaModel = $this->addMediaFromUrl($singleMedia['url']);
            } catch (\Throwable $th) {
                \Log::error($th);
                return new Media();
            }
        } elseif (isset($singleMedia['path'])) {
            $mediaModel = $this->addMediaFromDisk(
                $singleMedia['path'],
                $singleMedia['disk'] ?? null,
            );
        } else {
    ...
    public function getPanelMedia(
        string $collectionName = 'default',
        string $conversion = '',
        array|callable $filters = [],
    ) {
        return $this->getMedia($collectionName, $filters)->map(function (
            $singleMedia,
        ) use ($conversion) {
            $data = [
                'id' => $singleMedia->id,
    ...
    public function getMediaUrlsConversions(
        string $collectionName = 'default',
        array $sizes = ['original'],
    ) {
        $media = $this->getMedia($collectionName)->map(function ($item) use (
            $sizes,
        ) {
            foreach ($sizes as $s) {
                if ($s == 'original') {
